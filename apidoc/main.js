...
require([
    ...
    //
    var newList = [];
    var umlauts = { 'ä': 'ae', 'ü': 'ue', 'ö': 'oe', 'ß': 'ss' }; // : remove in version 1.0
    ...
    apiGroups.forEach(function(groupEntry) {
        ...
        api.forEach(function(entry) {
            if(groupEntry === entry.group) {
                ...
                // : make groupDescription compareable with older versions (not important for the moment)
    ...
    function changeVersionCompareTo(e) {
        ...
        if (compareVersion && articleVersions[group][name][0] === selectedVersion || version === selectedVersion) {
            ...
            // add unique id
            // : replace all group-name-version in template with id.
            ...
            $root.remove();
            // : on change main version or select the highest version re-render
        }
