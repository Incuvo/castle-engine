...
class Leaderboard
  ...
  ###
  constructor: (@leaderboardName, options = DEFAULT_OPTIONS, @redisConnection) ->
  ...
  # @param callback Optional callback for result of call.
  ###
  leaderboardExistsNamed: (leaderboardName, callback) ->

    @redisConnection.exists leaderboardName, (err, reply) ->
        callback(reply) if callback
  ...
  # @param callback Optional callback for result of call.
  ###
  leaderboardTTLNamed: (leaderboardName, callback) ->

    @redisConnection.ttl leaderboardName, (err, reply) ->
        callback(reply) if callback
  ...
  rankMemberIn: (leaderboardName, member, score, memberData = null, callback) ->
    ...
    transaction.exec((err, reply) ->
      callback(err, reply) if callback)
  ...
  # @param callback Optional callback for result of call.
  ###
  leaderboardIncCountNamed: (leaderboardName, incVal = 1, callback) ->
    @redisConnection.hincrby( this.memberDataKey(leaderboardName), '__global_count__', incVal, (err, reply) ->
      callback(err, reply) if callback)
        ...
        do (member) =>
          ...
          if options['with_member_data']
            this.memberDataForIn leaderboardName, member, (memberdata) =>
              ...
              if ranksForMembers.length == validMembers.length
  ...
  leadersIn: (leaderboardName, currentPage, options = {}, callback) ->
    ...
    this.totalMembersIn leaderboardName, (totalMembers) =>

      totalPages = Math.ceil( totalMembers / pageSize )

      if currentPage > totalPages
        return callback { total:totalMembers, results: []}
      ...
      if @reverse
        @redisConnection.zrange leaderboardName, startingOffset, endingOffset, (err, reply) =>
          this.rankedInListIn leaderboardName, reply, options, (replay) ->
            callback { total:totalMembers, results: replay}
